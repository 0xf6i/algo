...
from lab_a.sort.merge import merge
...
from lab_a.sort.quicksort.lomuto import lomuto
...
def test_radix_strings():
    from lab_a.sort.radix.strings import strings
    input_arr = ["apple", "banana", "cherry", "date"]
    # Shuffled version
    shuffled = ["cherry", "apple", "date", "banana"]
    result = strings(shuffled)
    actual = result if result is not None else shuffled
    assert actual == ["apple", "banana", "cherry", "date"]
